...
- 📄 **Upload** — upload PDF study material from the React UI.
- 💬 **Chat** — ask questions about your uploaded documents and get grounded, context-aware answers.
- 🕘 **History** — view previously uploaded documents.
- 🔐 **Per-user isolation** — pages are tagged with the uploader's email, so a user's questions only ever retrieve context from their own documents.
- ⚙️ **Asynchronous ingestion** — page-level text extraction and embedding happen off the request path via a background worker and a Valkey queue, so uploads return quickly.
...
**History:** frontend → backend → list of the user's previously uploaded documents.
...
| Infra | MinIO, Valkey, PostgreSQL + pgvector, Docker Compose |
...
For per-service environment variables, local dev setup (without Docker), and API references, see each service's README linked in the table above.
